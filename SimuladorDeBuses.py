...
def empezarSimul(Datosempleados, DatosAtencionMax, DatosAtencionMin):
    json_params = {"Datos_por_dia": list()}
    ...
    for i in range(DIAS):
        ...
        #json_params["Datos_por_dia"].append({'Longitud promedio de la cola:': lpc})
        print("\nLongitud promedio de la cola: %.2f" % lpc)

        tep = TIEMPO_ESPERA_TOTAL / PASAJEROS
        #json_params["Datos_por_dia"].append({'Tiempo de espera promedio:': tep})

        print("Tiempo de espera promedio = %.2f" % tep)
        upi = (DURACION_SERVICIO_TOTAL / TIEMPO_FINALIZACION) / \
            EMPLEADOS_VENTANILLA
        #json_params["Datos_por_dia"].append({'Uso promedio de la instalacion:': upi})
        print("Uso promedio de la instalacion = %.2f" % upi)
        print("\n---------------------------------------------------------------------")

        json_params["Datos_por_dia"].append({'Dia':i,'Longitud promedio de la cola:': lpc,
        'Tiempo de espera promedio:': tep,'Uso promedio de la instalacion:': upi})
    ...
    with open('Resultados.json', 'w') as outfile:
        json.dump(json_params, outfile, indent=4, cls=NumpyArrayEncoder)
